...
| `agg_func` | `str / null` | 聚合函数：`max`、`min`、`max_n`、`min_n`、`avg`、`variance`、`sum`、`count`；为 `null` 时表示普通条件查询 |
...
| `limit` | `int / null` | 返回结果数量限制，如 `5`、`10`；为 `null` 表示不限制 |
...
4. **`limit`** → SQL 的 `LIMIT` 子句（仅对 `max_n`/`min_n` 和普通过滤生效）
